...
# Shared hand / finger constants to avoid repetition
...
# Flattened list of finger chains (for iteration)
FINGER_CHAINS = list(RIGHT_FINGERS.values()) + list(LEFT_FINGERS.values())


def _contiguous_false_runs(b: np.ndarray) -> list:
    """Return list of (start, end) indices for contiguous False runs in boolean 1D array b.
...
def pchip_fill_tiny_gaps(X: np.ndarray, M: np.ndarray, max_gap: int) -> Tuple[np.ndarray, np.ndarray]:
    """Improved PCHIP implementation with bounds checking and spatial constraints

    This implementation fills only small gaps (<= max_gap) and is intended
    to be the primary PCHIP-based filler used by the pipeline.
    """
    try:
    ...
    for j in range(J):
        obs = M[:, j]
        # Need at least some observed points to interpolate
        if obs.sum() < 2:
            continue
        ...
        for c in range(C):
            ...
            # Iterate over contiguous False runs (gaps)
            for (s, e) in _contiguous_false_runs(obs):
                gap_len = e - s + 1
                if gap_len > max_gap:
                    continue
                ...
                if s > 0 and e < (T - 1):
                    try:
                        ...
                        # Check for unreasonable jumps from previous frame
                        prev_val = X_out[s-1, j, c]
                        max_jump = max(1e-8, y_range * 2.0)
                        if abs(y_interp[0] - prev_val) > max_jump:
                            # Use linear interpolation instead for this gap
                            y_interp = np.linspace(prev_val, X_out[e+1, j, c], gap_len)

                        # Temporal smoothing (simple moving average)
                        if gap_len > 2:
                            y_smooth = y_interp.copy()
                            for ii in range(1, gap_len-1):
                                y_smooth[ii] = 0.25 * y_interp[ii-1] + 0.5 * y_interp[ii] + 0.25 * y_interp[ii+1]
                            y_interp = y_smooth
                    ...
                    # Edge gap - use linear extrapolation when possible
...
def compute_target_bone_lengths(X: np.ndarray, M: np.ndarray) -> Dict[Tuple[int, int], float]:
    """Compute typical bone lengths from observed frames.

    Returns a dict mapping (parent_idx, child_idx) -> median_length.
    """
    targets = {}
...
def project_bone_lengths(X: np.ndarray, imputed: np.ndarray,
                         target_lengths: Dict[Tuple[int, int], float]) -> np.ndarray:
    """Project imputed child joints to match target bone lengths."""
    Xp = X.copy()
...
def clean_micro_gaps(X: np.ndarray, max_gap: int = 5) -> Tuple[np.ndarray, Dict]:
    ...
    for j in range(M.shape[1]):
        ...
        if joint_mask.sum() < joint_mask.shape[0]:  # Has gaps
            gaps_found += 1
            # Count fillable gaps using helper
            for (s, e) in _contiguous_false_runs(joint_mask):
                if (e - s + 1) <= max_gap:
                    total_gaps += 1
...
def main():
    ...
    print(f"Using built-in recovery implementation")
    print("=" * 60)
